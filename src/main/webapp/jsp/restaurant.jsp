...
<head>
    ...
    <title><%= (restaurant != null) ? restaurant.getRestaurantName() : "Restaurant" %> - FoodExpress</title>
...
<body>
    ...
    <main class="container page-wrapper">
        ...
        <% if (restaurant != null) { %>
            <!-- Restaurant Info Hero Banner -->
            <section class="restaurant-hero">
                <img src="<%= restaurant.getImage() %>" alt="<%= restaurant.getRestaurantName() %>" class="restaurant-hero-img">
                <div class="restaurant-hero-details">
                    <h1 class="headline-lg" style="color: var(--color-on-surface);"><%= restaurant.getRestaurantName() %></h1>
                    <p class="body-md" style="color: var(--color-on-surface-variant);"><%= restaurant.getDescription() %></p>
                    
                    <div class="restaurant-hero-meta">
                        <span class="badge badge-rating">
                            <i class="fa fa-star" style="color: #ffcc00;"></i> <%= restaurant.getRating() %> (500+ ratings)
                        </span>
                        <span class="badge badge-delivery">
                            <i class="fa fa-clock"></i> <%= restaurant.getDeliveryTime() %> mins delivery time
                        </span>
                        <span class="badge badge-delivery" style="background-color: var(--color-surface-container-high);">
                            <i class="fa fa-phone"></i> <%= restaurant.getPhone() %>
                        </span>
                    </div>
                    
                    <p class="label-sm" style="color: var(--color-on-surface-variant); margin-top: var(--space-sm);">
                        <i class="fa fa-map-marker-alt" style="color: var(--color-primary);"></i> <%= restaurant.getAddress() %>
                    </p>
                </div>
            </section>
            ...
            <section class="menu-container">
                <%
                    if (menuList != null && !menuList.isEmpty()) {
                        ...
                        for (Menu menu : menuList) {
                            ...
                            if (!menu.getCategory().equals(currentCategory)) {
                                currentCategory = menu.getCategory();
                %>
                                <div class="category-section" style="margin-top: var(--space-sm);">
                                    <h2 class="headline-md category-title"><%= currentCategory %></h2>
                                </div>
                <%
                            }
                %>
                            ...
                            <div class="menu-card">
                                <div class="menu-img-wrapper">
                                    <img src="<%= menu.getImage() %>" alt="<%= menu.getFoodName() %>">
                                </div>
                                <div class="menu-details">
                                    <div>
                                        <div class="menu-header-row">
                                            <h3 class="menu-item-name"><%= menu.getFoodName() %></h3>
                                            <% if (menu.isVeg()) { %>
                                                <span class="badge-veg"><i class="fa fa-circle" style="font-size: 8px;"></i> VEG</span>
                                            <% } else { %>
                                                <span class="badge-nonveg"><i class="fa fa-circle" style="font-size: 8px;"></i> NON-VEG</span>
                                            <% } %>
                                        </div>
                                        <p class="menu-item-desc"><%= menu.getDescription() %></p>
                                    </div>
                                    <div class="menu-bottom-row">
                                        <span class="menu-item-price">Rs. <%= menu.getPrice() %></span>
                                        <% if (menu.isAvailable()) { %>
                                            <button onclick="addToCart('<%= menu.getMenuId() %>')" class="btn btn-primary btn-sm">
                                                Add to Basket <i class="fa fa-cart-plus"></i>
                                            </button>
                                        <% } else { %>
                                            <span class="label-sm" style="color: var(--color-error); font-weight: 600;">Unavailable</span>
                                        <% } %>
                                    </div>
                                </div>
                            </div>
                ...
                %>
                    <div style="text-align: center; padding: var(--space-xl) 0;">
                        <i class="fa fa-bowl-food" style="font-size: 48px; color: var(--color-outline-variant);"></i>
                        <p class="body-md" style="margin-top: 10px;">This restaurant does not have any menu items available right now.</p>
                    </div>
                <%
                    }
                %>
        ...
        <% } else { %>
            <div style="text-align: center; padding: var(--space-xl) 0;">
                <i class="fa fa-exclamation-triangle" style="font-size: 48px; color: var(--color-error);"></i>
                <p class="body-md" style="margin-top: 10px;">Restaurant details not found.</p>
            </div>
        <% } %>
    </main>
