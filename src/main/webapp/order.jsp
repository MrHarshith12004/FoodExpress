...
<body>
    ...
    <main class="container page-wrapper">
        ...
        <!-- Back Link -->
        <a href="${pageContext.request.contextPath}/home" class="label-md" style="display: inline-flex; align-items: center; gap: var(--space-xs); margin-bottom: var(--space-sm); color: var(--color-primary); font-weight: 600;">
            <i class="fa fa-arrow-left"></i> Go to Restaurants
        </a>

        <!-- Success Toast/Card -->
        <% if (successMessage != null) { %>
            <div class="alert alert-success" style="padding: var(--space-md); margin-bottom: var(--space-md);">
                <i class="fa fa-circle-check" style="font-size: 24px;"></i>
                <div>
                    <h3 class="label-md" style="font-weight: 700; margin-bottom: 2px;"><%= successMessage %></h3>
                    <p class="label-sm" style="font-weight: 400;">Your order <strong>#<%= newOrderId %></strong> has been received and sent to the kitchen. You can track it below.</p>
                </div>
            </div>
        <% } %>

        <h1 class="headline-lg" style="margin-bottom: var(--space-md); color: var(--color-on-surface);">My Orders</h1>
                    ...
                    <div class="order-history-card">
                        ...
                        <div class="order-history-header">
                            <div>
                                ...
                                <div class="order-date"><i class="fa fa-calendar-days"></i> <%= dateFormat.format(order.getOrderDate()) %></div>
                            </div>
                            <div>
                        ...
                        <% if (rest != null) { %>
                            <div style="display: flex; align-items: center; gap: var(--space-sm); margin-top: 4px;">
                                <img src="<%= rest.getImage() %>" alt="<%= rest.getRestaurantName() %>" style="width: 45px; height: 45px; object-fit: cover; border-radius: var(--radius-default);">
                                <div>
                                    <h4 class="label-md" style="font-weight: 600;"><%= rest.getRestaurantName() %></h4>
                                    <p class="label-sm" style="color: var(--color-on-surface-variant); font-weight: 400;"><%= rest.getAddress() %></p>
                                </div>
                            </div>
                        <% } %>

                        <!-- Order items list -->
                        <div class="order-details-summary" style="margin-top: 10px; padding: var(--space-sm); background-color: var(--color-surface-container-lowest); border-radius: var(--radius-md); border: 1px solid var(--color-surface-container-high);">
                            <%
                                if (items != null && !items.isEmpty()) {
                                    for (OrderItem item : items) {
                                        Menu menu = menuDAO.getMenuById(item.getMenuId());
                                        String foodName = (menu != null) ? menu.getFoodName() : "Unknown Item";
                            %>
                                        <div style="display: flex; justify-content: space-between; font-size: 13px;">
                                            <span><%= foodName %> <strong style="color: var(--color-primary);">x<%= item.getQuantity() %></strong></span>
                                            <span>Rs. <%= item.getSubtotal() %></span>
                                        </div>
                            <%
                                    }
                                }
                        ...
                        <div style="font-size: 12px; color: var(--color-on-surface-variant); margin-top: 4px; display: flex; flex-direction: column; gap: 2px;">
                            <span><strong>Delivery Address:</strong> <%= order.getDeliveryAddress() %></span>
                            <span><strong>Payment Method:</strong> <%= order.getPaymentMethod() %></span>
                        </div>

                        <!-- Order Footer (Total) -->
                        <div class="order-history-footer">
                            <span class="body-sm" style="color: var(--color-on-surface-variant);">Grand Total</span>
                            <span class="order-total-amount">Rs. <%= order.getTotalAmount() %></span>
                        </div>
        ...
        %>
            <!-- Empty Order State -->
            <div style="text-align: center; padding: 60px var(--space-md); background: #ffffff; border-radius: var(--radius-lg); box-shadow: var(--shadow-level-1);">
                <i class="fa fa-receipt" style="font-size: 64px; color: var(--color-outline-variant); margin-bottom: 20px;"></i>
                <h2 class="headline-md" style="margin-bottom: var(--space-xs);">No orders found!</h2>
                <p class="body-md" style="color: var(--color-on-surface-variant); margin-bottom: var(--space-md); max-width: 400px; margin-left: auto; margin-right: auto;">
                    You haven't placed any orders yet. Try one of our delicious cuisines to fill your plate!
                </p>
